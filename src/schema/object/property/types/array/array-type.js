...
function resolve({ property, config }) {
  return ArrayType.create({ property, config }).apply();
}
...
class ArrayType extends PrimitiveType {
  ...
  get typeName() {
    return this.hasSingleType ? this.firstTypeName : this.unionTypeName;
  }
  ...
  get typeNames() {
    this._typeNames = this._typeNames || this.itemsResolver.resolve();
