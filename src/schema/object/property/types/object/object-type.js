const { FingerprintedType } = require("../finger-printed");
...
function resolve({ property, config }) {
  return (
    isObject(property) && ObjectType.create({ property, config }).resolve()
  );
}

// Allow recursive schema
class ObjectType extends FingerprintedType {
  constructor({ property, config, opts }) {
    ...
    this.createObjectTypeNameResolver =
      (config || {}).createObjectTypeNameResolver ||
  ...
  resolveTypeName() {
    this.objectTypeNameResolver = this.createObjectTypeNameResolver({
      object: this,
      config: this.config
    });
    return this.objectTypeNameResolver.resolve();
  }
  ...
  resolveNested() {
    if (!this.shouldResolveNested) return this;
    ...
    const propsResolver = this.createPropertiesResolver({
      object,
      config: this.config
    });
  ...
  get valid() {
    return this.properties || this.reference ? true : false;
  }
}
